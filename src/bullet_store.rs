...
use small_gl_core::{gl, SIZE_OF_FLOAT, size_of_floats};
...
impl BulletStore {
    pub fn initialize_buffer_and_create(/* threadpool */) -> Self {
        ...
        unsafe {
            ...
            gl::BufferData(gl::ARRAY_BUFFER, size_of_floats!((bulletVertices.len())) as GLsizeiptr, bulletVertices.as_ptr() as *const GLvoid, gl::STATIC_DRAW);
            gl::BindBuffer(gl::ELEMENT_ARRAY_BUFFER, bullet_ebo);
            gl::BufferData(gl::ELEMENT_ARRAY_BUFFER, size_of_floats!(bulletIndices.len()) as GLsizeiptr, bulletIndices.as_ptr() as *const GLvoid, gl::STATIC_DRAW);
    ...
    pub fn create_bullets(&mut self, position: Vec3, midDir: Vec3, spreadAmount: i32) {
        ...
        for p in 0..parallelism {
            ...
            for i in iStart..iEnd {
                let yQuat = midDirQuat * Quat::from_axis_angle(
                    vec3(0.0, 1.0, 0.0),
                    rotPerBullet * ((i - spreadAmount) as f32 / 2.0),
                );

                for j in 0..spreadAmount {
                    let rotQuat = yQuat * Quat::from_axis_angle(
                        vec3(1.0, 0.0, 0.0),
                        rotPerBullet * ((j - spreadAmount) as f32 / 2.0),
                    );
